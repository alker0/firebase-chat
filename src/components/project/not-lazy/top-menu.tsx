  ...
  createComponent: (contextArg?: TopMenu.Context): Component<TopMenu.Props> => {
    ...
    return props => {
      return <div class={cn('content')}>
        ...
        <div class={cn('row')}>
          <button class={cn('btn-xlarge', 'offset-center')} onClick={context.onSessionButtonClick}>
            {context.getSessionButtonText()}
          </button>
        </div>
        <div class={cn('row')}>
          <div class={cn('col-6', 'ignore-screen')}><div class={cn('btn-container')}>
            <div class={cn('btn', 'btn-xlarge', 'btn-animated')} onClick={context.onLeftButtonClick}>{context.leftButtonText}</div>
          </div></div>
          <div class={cn('col-6', 'ignore-screen')}><div class={cn('btn-container')}>
            <div class={cn('btn', 'btn-xlarge', 'btn-animated')} onClick={context.onRightButtonClick}>
