...
export const BasicInputField = {
  createComponent(contextArg: BasicInputField.Context = {}): Component<BasicInputField.Props> {
    const context = assignProps({}, defaultContext, contextArg);

    const [sizedLabel, sizedInput] = (['label', 'input'] as const)
      .map(item => {
    ...
    return propsArg => {
      ...
      return <div {...props.ofWrapper} class={clsx(wrapperStyle, props.ofWrapper.class)}>
        <div children={props.labelText} {...props.ofLabel} class={clsx(labelStyle, sizedLabel, props.ofLabel.class)} />
        <input {...context.baseInputProps} {...props.ofInput} class={clsx(sizedInput, props.ofInput.class)} />
      </div>;
    };
  }
};

export const createComponent = BasicInputField.createComponent;
