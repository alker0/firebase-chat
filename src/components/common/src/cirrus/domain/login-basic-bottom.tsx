...
const defaultContext: DefaultComponents<LoginBasicBottom.Context, DefaultPropsMap> = {
  wrapper: props => <div {...props} class={clsx(defaultWrapperStyle, props.class)} />,
...
export const LoginBasicBottom = {
  ...
  >(contextArg: LoginBasicBottom.Context<T, U, V> = {}): Component<LoginBasicBottom.Props<T, U, V>> {
    const context = assignProps({}, defaultContext, contextArg);

    return propsArg => {
      const props = assignProps({}, defaultProps, propsArg);

      return <context.wrapper {...props.ofWrapper}>
...
export const createComponent = LoginBasicBottom.createComponent;
