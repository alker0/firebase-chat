...
while (targets.length) {
  ...
  const processor = (async () => {
    ...
    return await previous.then(_ => {
      ...
      return postcss(plugins.concat(classNameDeclarer({dest: targetTypeOutput})))
        ...
        .then(_ => {
          ...
          return new Promise(resolve => setTimeout(resolve, 250, relativeOutput))
        })
        .catch(console.error)
    })
  })()
...
Promise
.allSettled(processors)
.then(results => results
    .slice(1)
    .filter(result => result.status === 'fulfilled' && fsSync.existsSync(result.value))
    .map(result => result.value)
    .forEach(declaredPath => console.log(greenNormal, '🎉 Declared', ` : ${declaredPath}`))
)
.catch(console.error)
