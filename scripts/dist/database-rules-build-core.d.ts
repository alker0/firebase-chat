...
export declare const ruleValue: (currentValue: string, { isCaptured, isNum, isBool, isStringLiteral, }?: {
    isCaptured?: boolean;
    isNum?: boolean;
    isBool?: boolean;
    isStringLiteral?: boolean;
...
export declare const joinArrayValues: (values: RuleValueArgs) => string, joinPaths: (paths: RuleValueArgs) => string, joinTexts: (literalArgs: Literals, ...ruleValueArgs: RuleValueArgs) => RuleValue;
...
export declare const query: {
    orderByChild: (firstPath: RuleValueArg | null, ...paths: RuleValueArgs) => RuleValue;
...
export declare const extractText: (text: RuleExpArg) => string | number | boolean;
export declare function getRuleResultFromRuleNode(ruleNode: RuleExpArg | (RuleExpArg | RuleExpArgs)[], withBracket: boolean): string;
export declare type RuleKey = typeof read | typeof write | typeof validate;
declare type IndexOnKey = typeof indexOn;
declare type ChildRuleObjectKey<T extends {}> = Extract<Exclude<keyof T, RuleKey | IndexOnKey>, string>;
declare type SourceRuleValue = boolean | string | RuleValue | string[];
declare type SourceIndexOnValue = (string | RuleValue)[];
export declare type SourceRuleObject<T extends {}> = Record<keyof T & RuleKey, SourceRuleValue> & Record<keyof T & IndexOnKey, SourceIndexOnValue> & {
    [K in ChildRuleObjectKey<T>]: K extends ChildRuleObjectKey<T> ? T[K] : never;
};
declare type ResultRuleValue = string | boolean;
declare type ResultIndexOnValue = string[];
export declare type ResultRuleObject<T extends Record<string, any>> = Record<RuleKey, ResultRuleValue> & Record<IndexOnKey, ResultIndexOnValue> & {
    [K in ChildRuleObjectKey<T>]: K extends ChildRuleObjectKey<T> ? ResultRuleObject<T[K]> : never;
};
export declare function createRuleObject<T extends {}, U extends ResultRuleObject<T>>(srcObj: T & SourceRuleObject<T>): U;
export declare const join: (...texts: RuleExpArgs) => string;
export declare const bracket: (...texts: RuleExpArgs) => string;
export declare const exp: (literalArgs: Literals, ...jsArgs: RuleExpArgs) => string;
